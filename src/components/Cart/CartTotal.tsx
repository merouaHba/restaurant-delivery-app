...
const CartTotal = ({checkoutState}: {checkoutState:any}) => {
  ...
  return (
    <div className='w-full mt-2 md:mt-0 flex-1 rounded bg-cartItem rounded-t-[2rem] px-8 py-2 flex flex-col items-center justify-evenly'>
        ...
        <motion.button onClick = {() => checkoutState(true)} whileTap={{scale:0.8}} className='w-full p-2 rounded-full bg-gradient-to-tr from-red-400 to-red-600 text-gray-50 text-lg my-2 hover:shadow-lg'>
          Checkout ₵{cartTotal}
        </motion.button>
